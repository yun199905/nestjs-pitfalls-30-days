...
一個文章更新 API 已經寫好了。Controller 的 `@Body()` 有型別，DTO 中也清楚宣告了 `string`、`string[]`、`number[]` 與巢狀物件；實際送出 request 時，NestJS 也能收到完整 JSON。然而打開 Swagger UI，request schema 卻少了欄位、把陣列的元素型別寫錯、把巢狀 model 顯示成空泛的 `object`，甚至整支端點連 request body 都不見了。

**這一題的每一支 broken 端點只示範一個坑。** 每支 broken DTO 都是同一份欄位集的複本，只刻意壞掉其中一處，其餘一律寫成正確版本 —— 所以你比對 schema 時，diff 永遠只會有一個地方不同，症狀可以被單獨歸因。四支 broken 端點共用同一支 correct 端點，因為正解只有一種。

| 端點                                  | 只示範這一個坑                                             |
| ------------------------------------- | ---------------------------------------------------------- |
| `PATCH /posts/{id}/missing-decorator` | 問題一：property 有 TS 型別，但沒掛 `@ApiProperty()`       |
| `PATCH /posts/{id}/untyped-array`     | 問題二：array 沒指定 items 型別                            |
| `PATCH /posts/{id}/nested-interface`  | 問題三：巢狀型別宣告成 `interface`                         |
| `PATCH /posts/{id}/plain-partial`     | 問題四：拿 TypeScript 的 `Partial<T>` 當 body 型別         |
| `PATCH /posts/{id}/correct`           | 正解：四個坑都修好的 `UpdatePostDto`                       |

五支端點的 runtime 行為**完全一樣**：同一份 JSON 送進去，回來的都是完整內容。差別只在 OpenAPI 文件。
...
五支端點共用這一組欄位。正確版長這樣（`src/dto/request/post-fields.dto.ts`），下面每個坑都是它的複本，只壞掉一處：
...
從 TypeScript 的角度看，五個 properties 都有型別。`content` 也照樣會被 handler 收到，因為 `ValidationPipe` 沒有開 `whitelist`，不會刪除任何欄位。

### 問題二：`src/dto/request/broken-untyped-array.dto.ts`

```typescript
// 兩個 array 都只掛裸 @ApiProperty()，其餘欄位一律正確
...
decorator 有掛、TypeScript 型別也有寫，看起來完全沒問題。

### 問題三：`src/dto/request/broken-nested-interface.dto.ts`
...
### 問題四：`src/dto/request/broken-plain-partial.dto.ts`
...
# ["title","content","tags","relatedPostIds","publishOptions"]
```

### 問題一：欄位在 schema 裡完全不存在

```bash
curl -s localhost:3000/api-json \
  | jq '.components.schemas.MissingDecoratorUpdatePostDto.properties | keys'
# ["title","tags","relatedPostIds","publishOptions"]
```

`content` 不是「顯示成錯的型別」，而是**根本不存在**。其餘四個欄位一切正常，包含 `publishOptions` 的 `$ref`。
...
### 問題二：`string[]` 剛好對，`number[]` 默默錯

```bash
curl -s localhost:3000/api-json \
  | jq '.components.schemas.UntypedArrayUpdatePostDto.properties
        | {tags, relatedPostIds}'
```
...
```json
{
  ...
  "relatedPostIds": { "type": "array", "items": { "type": "string" } }
}
```

這是這一題最容易被誤讀的一個坑，值得盯著看清楚：

- `tags: string[]` 被寫**對**了。
- `relatedPostIds: number[]` 也被寫成了 `items: { type: 'string' }`。

兩個欄位的宣告方式一模一樣，schema 也一模一樣 —— 所以 `tags` 對，不是因為 Swagger 看懂了 `string[]`。文件上這支 API 收的是字串陣列，Postman 使用者會照著送 `["23","24"]`，而 TypeScript 和後端都以為自己拿到 `number[]`。整條路上沒有任何人會抱怨。

### 問題三：巢狀 model 只剩沒有 properties 的空殼
...
`publishOptions` 有出現，但 Swagger 不知道裡面還有 `notifyFollowers`，因此只顯示一般 `object`；Swagger UI 的 Example Value 只會給你一個 `{}`。對照同一支 DTO 的其他四個欄位 —— 它們都是正確的，包含 `relatedPostIds` 的 `items: { type: 'number' }`。

### 問題四：整支端點連 request body 都不見了
...
每支 broken 端點只壞在一個地方，請逐支完成 Debug：

**問題一（`/missing-decorator`）**

1. 找出 `content` 從 OpenAPI schema 消失的原因，並說明為什麼 runtime 還是收得到它。
2. 說明 `PartialType()` 實際從 base DTO 繼承了什麼，而不是只回答「它把型別變成 optional」——為什麼它補不出 `content`？

**問題二（`/untyped-array`）**

3. `tags` 和 `relatedPostIds` 的宣告方式一模一樣，為什麼一個對、一個錯？先回答「`tags` 為什麼是對的」。
4. 讓 array 的元素型別在 OpenAPI schema 裡正確呈現。

**問題三（`/nested-interface`）**

5. 解釋 `publishOptions` 為什麼只能顯示成沒有 properties 的 `object`，且 `components.schemas` 裡找不到它。

**問題四（`/plain-partial`）**

6. base DTO 的 metadata 是完整的，為什麼這支端點連 `requestBody` 都沒有產生？
7. 確認 Swagger DTO 情境下 `PartialType()` 應該從哪個 package 匯入，並說明它和 TypeScript 的 `Partial<T>` 差在哪。

**整體**

8. 提出一組 class-based DTO，使 update schema 完整，且全部頂層欄位維持 optional。
...
比較 `title` 與 `content` 的宣告。兩者都有 TypeScript 的 `string`，但只有其中一個在 class prototype 上登記了 Swagger 需要的 property metadata。
...
`emitDecoratorMetadata` 對 `string[]` 和 `number[]` 留下的 `design:type` 都只是 `Array` —— 元素型別在編譯後就沒了。既然 Swagger 拿不到元素型別，它一定得挑一個預設值。去 `node_modules/@nestjs/swagger/dist/services/schema-object-factory.js` 搜 `defaultOnArray`，你會知道它挑了什麼，也會馬上明白 `tags` 為什麼是對的。
...
- 應用程式可以正常 compile、啟動。
- 五支 PATCH 端點都能在 runtime 收到完整 JSON，回傳內容一致（只有 `variant` 不同）。

**問題一**

- `content` 出現在 schema 中，型別為 `string`。

**問題二**

- `tags` 是 `items: { type: 'string' }` 的 array schema。
- `relatedPostIds` 是 `items: { type: 'number' }` 的 array schema，不再是 string。
- 能說明修好之前 `tags` 為什麼看起來是對的。
...
自動化測試會逐坑檢查上述 OpenAPI schema 差異，並確認五支端點的 runtime response 一致。
...
關鍵字是「已有」。如果 base DTO 沒有登記 `content`，mapped type 根本不知道要複製它。所以問題一的症狀不是 `PartialType()` 的鍋，是它上游的鍋 —— 這也是為什麼這兩件事不該分成兩支端點來示範。

### 問題二：Swagger 對 array 的元素型別只能猜，而它猜 string

`emitDecoratorMetadata` 對 `string[]` 和 `number[]` 留下的 `design:type` 都是 `Array`，元素型別編譯後就沒了。`@nestjs/swagger` 遇到這種情況會直接套用預設值：

```javascript
// node_modules/@nestjs/swagger/dist/services/schema-object-factory.js
else if (itemType === 'array') {
    const defaultOnArray = 'string';
    ...
    return this.transformToArraySchemaProperty(metadata, key, { type: defaultOnArray });
}
```

所以 `tags: string[]` 是**歪打正著**：Swagger 沒有看懂它，只是預設值剛好對上。同樣的預設值套在 `number[]`、`boolean[]` 或任何巢狀 DTO array 上，就會默默寫錯，而且不會有任何警告。
...
`tags` 那行照樣要寫。它現在剛好對，只是因為預設值撞對了，不是因為你表達了意圖。

### 問題三：interface 在 runtime 不存在
...
### 問題四：`Partial<T>` 與 `PartialType()` 是兩種完全不同的東西
...
四個坑的共同根因是同一件事：**沒有人負責把上一層的資訊帶到下一層。** 問題一是沒登記、問題二是登記得不夠具體、問題三是沒有東西可以登記、問題四是登記在拿不到的地方。
